...
def snapshot_raw_version(input_path, registry_dir):
    ...
    # 2. Look for existing version with the same content hash
    raw_versions_dir = os.path.join(registry_dir, "raw_versions")
    if os.path.exists(raw_versions_dir):
    ...
    # 3. Allocate a new version id
    new_version_id = _next_version_id(raw_versions_dir)

    # 4. Create the directory and write manifest.json
    new_version_dir = os.path.join(raw_versions_dir, new_version_id)
    ...
    manifest_path = os.path.join(new_version_dir, "manifest.json")
    with open(manifest_path, "w") as f:
        json.dump(manifest, f, indent=2)

    # 5. Return the new version_id
...
def build_features(rows):
    ...
    for row in rows:
        ...
        if card_id not in card_groups:
            card_groups[card_id] = []
        card_groups[card_id].append(row)

    feature_rows = []
    for card_id, transactions in card_groups.items():
        ...
        for txn in transactions:
            ...
            if "amount" in txn:
                ...
                amount = float(int(txn["amount_minor_units"]) / 100)

            total_amount += amount
            max_amount = max(max_amount, amount)

            if txn.get("card_present") == "True":
                card_present_count += 1

            event_time = txn["timestamp"]
            if event_time > max_event_time:
                max_event_time = event_time

        avg_amount = round(total_amount / txn_count, 2) if txn_count > 0 else 0.0
        pct_card_present = round(card_present_count / txn_count, 3) if txn_count > 0 else 0.0
...
def register_feature_group(name, feature_rows, source_version_id, registry_dir, transform_version="v1"):
    ...
    # 2. Create that directory
    fg_version_dir = os.path.join(feature_group_dir, fg_version_id)
    os.makedirs(fg_version_dir, exist_ok=True)

    # 3. Write features.json
    features_path = os.path.join(fg_version_dir, "features.json")
    ...
    manifest_path = os.path.join(fg_version_dir, "manifest.json")
    with open(manifest_path, "w") as f:
        json.dump(manifest, f, indent=2)

    # 5. Return fg_version_id
...
def get_lineage(name, fg_version_id, registry_dir):
    ...
    """
    # 1. Read the feature group's manifest
    feature_group_dir = os.path.join(registry_dir, "feature_groups", name)
    fg_version_dir = os.path.join(feature_group_dir, fg_version_id)
    feature_group_manifest_path = os.path.join(fg_version_dir, "manifest.json")
    with open(feature_group_manifest_path, "r") as f:
        feature_group_manifest = json.load(f)

    # 2. Read the raw source's manifest
    source_raw_version_id = feature_group_manifest["source_raw_version_id"]

    raw_source_manifest_path = os.path.join(
        registry_dir,
        "raw_versions",
